...
void do_autonomous() {
    Serial.println("Autonomous Now");
    int start_time = millis(); // Get the time in milliseconds
    while (millis() - start_time < 20000 && !RR_buttonB()) {   //press B to stop autonomous
      float base_speed = 0.23;  //test and change the parameter
      float gain       = 0.1;  //test and change the parameter
      
      int line_sensors[6];
      RR_getLineSensors(line_sensors);

      int line_sensor;
      int darkest_value = -1; // Make sure this is brighter than any possible reading
      for (int i = 0; i < 6; ++i) {
          // check if line_sensor[i] is darker than darkest_value
          // if it is, update line_sensor and darkest_value
          // this is left as an exercise for the reader :)
          if(line_sensors[i]>darkest_value){
            darkest_value = line_sensors[i];
            line_sensor = i;
          }
      }

      float error = line_sensor - 2.5;

      if(millis()-start_time % 100 == 0){
        Serial.println(error);
      }

      RR_setMotor3(base_speed + error * gain);
      RR_setMotor2(base_speed - error * gain);
    }
}
...
void loop() {
  if(RR_buttonA()){    //press A to go autonomous
    do_autonomous();
  }
